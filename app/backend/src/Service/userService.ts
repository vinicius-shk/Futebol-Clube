...
const loginService = async (body: ILoginBody): Promise<{ type: number | null, message: string }> => {
  const response = await User.findOne({ where: { email: body.email } });
  if (!response || !compareSync(body.password, response.password)) return { type: 401, message: 'Incorrect email or password' };
